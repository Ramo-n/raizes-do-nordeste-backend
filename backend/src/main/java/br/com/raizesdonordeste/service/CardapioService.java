...
public class CardapioService {
    ...
    public List<ItemCardapio> cardapioDaUnidade(Long unidadeId) {
        ...
        return produtoUnidadeRepository.findByUnidadeId(unidadeId).stream()
                .filter(pu -> pu.disponivelEm(hoje))
                .map(pu -> new ItemCardapio(pu.getProduto().getId(), pu.getProduto().getNome(),
                        pu.getProduto().getDescricao(), pu.getProduto().getCategoria(),
                        pu.getPreco(), pu.getVariacaoRegional(), pu.isSazonal()))
                .toList();
    }
}
