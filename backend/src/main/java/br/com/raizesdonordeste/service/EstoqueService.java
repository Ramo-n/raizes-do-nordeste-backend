...
public class EstoqueService {
    ...
    public EstoqueItem ajustar(Long estoqueItemId, int novaQuantidade, String autor, String justificativa) {
        EstoqueItem item = estoqueItemRepository.findById(estoqueItemId)
                .orElseThrow(() -> new RecursoNaoEncontradoException("Item de estoque não encontrado: " + estoqueItemId));
        if (novaQuantidade < 0) {
            throw new NegocioException("Quantidade de estoque não pode ser negativa");
        }
        int anterior = item.getQuantidade();
        item.setQuantidade(novaQuantidade);
        auditoriaService.registrar(TipoOperacaoSensivel.AJUSTE_ESTOQUE, autor,
                "Ajuste de estoque do item " + estoqueItemId + " (" + item.getProduto().getNome()
                        + ") de " + anterior + " para " + novaQuantidade + " na unidade "
