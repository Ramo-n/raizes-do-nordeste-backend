...
public class PedidoService {
    ...
    public Pedido criarPedido(NovoPedido req) {
        ...
        if (req.clienteId() != null) {
            Cliente cliente = clienteRepository.findById(req.clienteId()).orElseThrow(() -> new RecursoNaoEncontradoException("Cliente não encontrado: " + req.clienteId()));
            pedido.setCliente(cliente);
        }
        LocalDate hoje = LocalDate.now();
        BigDecimal bruto = BigDecimal.ZERO;
        for (NovoItemPedido item : req.itens()) {
            ProdutoUnidade pu = produtoUnidadeRepository.findByUnidadeIdAndProdutoId(unidade.getId(), item.produtoId()).orElseThrow(() -> new NegocioException("Produto " + item.produtoId() + " não pertence ao cardápio da unidade " + unidade.getId()));
            if (!pu.disponivelEm(hoje)) {
                throw new NegocioException("Produto indisponível nesta unidade/período: " + pu.getProduto().getNome());
            }
            EstoqueItem estoque = estoqueItemRepository.findByUnidadeIdAndProdutoId(unidade.getId(), item.produtoId()).orElseThrow(() -> new NegocioException("Sem estoque cadastrado para o produto " + pu.getProduto().getNome()));
    ...
    public Pedido avancarStatus(Long pedidoId) {
        ...
        if (pedido.getStatus() == StatusPedido.PAGO) {
            proximo = StatusPedido.EM_PREPARO;
        } else if (pedido.getStatus() == StatusPedido.EM_PREPARO) {
            proximo = StatusPedido.PRONTO;
        } else if (pedido.getStatus() == StatusPedido.PRONTO) {
            proximo = StatusPedido.ENTREGUE;
        } else {
            throw new NegocioException("Não é possível avançar o pedido no status " + pedido.getStatus());
        }
    ...
    public Pedido cancelar(Long pedidoId, String autor, String justificativa) {
        ...
        boolean estavaConfirmado = pedido.getStatus() == StatusPedido.PAGO || pedido.getStatus() == StatusPedido.EM_PREPARO || pedido.getStatus() == StatusPedido.PRONTO;
        if (estavaConfirmado) {
            devolverEstoque(pedido);
        }
        pedido.setStatus(StatusPedido.CANCELADO);
        auditoriaService.registrar(TipoOperacaoSensivel.CANCELAMENTO, autor, "Cancelamento do pedido " + pedidoId, justificativa, pedidoId, pedido.getCliente() != null ? pedido.getCliente().getId() : null);
        return pedido;
    }

    private void devolverEstoque(Pedido pedido) {
        for (ItemPedido item : pedido.getItens()) {
            estoqueItemRepository.findByUnidadeIdAndProdutoId(pedido.getUnidade().getId(), item.getProduto().getId()).ifPresent(e -> e.setQuantidade(e.getQuantidade() + item.getQuantidade()));
        }
    }
    ...
    public Pedido aplicarDesconto(Long pedidoId, BigDecimal valorDesconto, String autor, String justificativa) {
        ...
        auditoriaService.registrar(TipoOperacaoSensivel.DESCONTO, autor, "Desconto de R$ " + valorDesconto + " no pedido " + pedidoId, justificativa, pedidoId, pedido.getCliente() != null ? pedido.getCliente().getId() : null);
        return pedido;
    }

    @Transactional(readOnly = true)
    public Pedido buscar(Long pedidoId) {
        return pedidoRepository.findById(pedidoId).orElseThrow(() -> new RecursoNaoEncontradoException("Pedido não encontrado: " + pedidoId));
    }
