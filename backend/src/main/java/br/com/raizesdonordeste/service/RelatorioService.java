...
public class RelatorioService {
    ...
    public List<VendasUnidade> vendasPorUnidade() {
        return pedidoRepository.vendasPorUnidade().stream()
                .map(r -> new VendasUnidade((Long) r[0], (String) r[1], (String) r[2],
                        (Long) r[3], (BigDecimal) r[4]))
                .toList();
    }
    ...
    public List<VendasRegiao> vendasPorRegiao() {
        return pedidoRepository.vendasPorRegiao().stream()
                .map(r -> new VendasRegiao((String) r[0], (Long) r[1], (BigDecimal) r[2]))
                .toList();
    }
    ...
    public List<ProdutoConsumo> produtosMaisConsumidos() {
        return pedidoRepository.produtosMaisConsumidos().stream()
                .map(r -> new ProdutoConsumo((Long) r[0], (String) r[1], (Long) r[2]))
                .toList();
    }
    ...
    public List<IndicadorUnidade> indicadoresDaUnidade(Long unidadeId) {
        BigDecimal vendas = vendasPorUnidade().stream()
                .filter(v -> v.unidadeId().equals(unidadeId))
                .map(VendasUnidade::total)
                .findFirst()
                .orElse(BigDecimal.ZERO);
        List<MetaUnidade> metas = metaUnidadeRepository.findByUnidadeId(unidadeId);
        return metas.stream()
                .map(m -> new IndicadorUnidade(unidadeId, m.getUnidade().getNome(), m.getAnoMes(),
                        m.getMetaVendas(), vendas,
                        m.getMetaVendas().signum() > 0
                                ? vendas.multiply(new BigDecimal("100"))
                                    .divide(m.getMetaVendas(), 2, RoundingMode.HALF_UP)
                                : BigDecimal.ZERO))
                .toList();
    }
}
