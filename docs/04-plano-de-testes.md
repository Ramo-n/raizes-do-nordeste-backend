...
|----|-----------|---------|--------------------|-----------|
| CT01 | RF01/RF05 | Consultar cardápio da unidade reduzida (SP) | Não lista produtos exclusivos de Recife (bolo de macaxeira, combo) | `LgpdECardapioTest` |
| CT02 | RF05 | Consultar cardápio fora/dentro do período junino | Canjica junina só aparece entre maio e julho | `LgpdECardapioTest` |
| CT03 | RF02/RF12/RF17 | Criar pedido de cliente com 120 pontos (2 tapiocas, R$24) | Pedido `AGUARDANDO_PAGAMENTO`, desconto 5% (R$1,20), pagamento solicitado | `FluxoPedidoTest` |
| CT04 | RF17/RF06/RF11 | Callback confirma o pagamento | Pedido `PAGO`, estoque baixado, pontos acumulados | `FluxoPedidoTest` |
| CT05 | RF18 | Callback reentregue (duplicado) | Nenhum efeito adicional (idempotência) | `FluxoPedidoTest` |
| CT06 | RF17/RF18 | Callback recusa o pagamento | Pedido `PAGAMENTO_RECUSADO`, estoque intacto | `FluxoPedidoTest` |
| CT07 | RF07 | Cancelar pedido pago | Pedido `CANCELADO`, estoque devolvido, registro de auditoria com autor/justificativa | `FluxoPedidoTest` |
| CT08 | RF07 | Aplicar desconto manual | Total recalculado e auditoria de DESCONTO gerada | `FluxoPedidoTest` |
| CT09 | RF01 | Pedir produto fora do cardápio da unidade | Erro 422 de negócio | `FluxoPedidoTest` |
| CT10 | RF06 | Pedir quantidade acima do estoque | Erro 422 "Estoque insuficiente" | `FluxoPedidoTest` |
| CT11 | RF14/RF11 | Registrar consentimento explícito | Consentimento com data/hora e conta de fidelidade criada | `LgpdECardapioTest` |
| CT12 | RF16 | Consultar dados pessoais de um cliente | Registro de auditoria `ACESSO_DADOS_PESSOAIS` | `LgpdECardapioTest` |
| CT13 | RF15 | Anonimizar cliente | Nome/e-mail/data de nascimento removidos irreversivelmente; auditoria `ANONIMIZACAO` | `LgpdECardapioTest` |
